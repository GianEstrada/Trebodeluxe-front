...
const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose, onGoToCheckout }) => {
  ...
  const handleDebugInfo = () => {
    console.log('🛒 Información de debugging del carrito:', getStorageInfo());
  };
          ...
          <div className="flex items-center justify-between border-b pb-4 mb-4">
            ...
            {items.length > 0 && (
              <button 
                ...
                disabled={isLoading}
                className="text-red-600 hover:text-red-700 text-sm border border-red-200 px-3 py-1 rounded disabled:opacity-50"
              >
                🗑️ Vaciar carrito
              </button>
            )}
              ...
              {items.map((item) => (
                <div key={item.id} className="flex items-start gap-4 p-4 border rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          disabled={isLoading}
                          className="w-8 h-8 border rounded text-center hover:bg-gray-100 disabled:opacity-50 flex items-center justify-center"
                        >
                          -
                        </button>
                      ...
                      <button
                        ...
                        disabled={isLoading}
                        className="text-red-600 hover:text-red-700 px-2 py-1 text-sm disabled:opacity-50"
                    ...
                    {/* Fecha agregado (solo en desarrollo) */}
                    {process.env.NODE_ENV === 'development' && (
                      <p className="text-xs text-gray-400 mt-1">
                        Agregado: {new Date(item.fechaAgregado).toLocaleString()}
                      </p>
                    )}
                  </div>
                </div>
