...
const HomeScreen: NextPage = () => {
  ...
  return (
    <div className="w-full relative min-h-screen flex flex-col text-left text-Static-Body-Large-Size text-M3-white font-salsa"
      ...
      {/* Sección de productos recientes por categoría */}
      {!loading && Object.keys(recentByCategory).length > 0 && (
        <div className="self-stretch bg-transparent flex flex-col items-center justify-start py-16 px-4">
          <div className="w-full max-w-7xl">
            <div className="mb-12 text-center">
              <h2 className="text-3xl font-bold text-white mb-4 tracking-[2px]">{t('NOVEDADES POR CATEGORÍA')}</h2>
              <p className="text-gray-300 text-lg">{t('Lo más reciente en cada categoría')}</p>
            </div>
            
            {Object.entries(recentByCategory).map(([category, products]: [string, any]) => (
              <div key={category} className="mb-12">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-2xl font-bold text-white tracking-[2px]">{t(category.toUpperCase())}</h3>
                  <Link 
                    ...
                    <Link key={product.id} href={`/producto/${product.id}`} className="no-underline">
                      <div className="bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20 hover:bg-white/20 transition-all duration-300 group">
                        ...
                        <div className="space-y-2">
                          <h3 className="text-white font-semibold text-sm group-hover:text-green-300 transition-colors duration-300 line-clamp-2">
                            {product.name}
                          </h3>
